...
Obsidianki supports **vector embedding deduplication** - if enabled, proposed flashcards by the LLM will be matched against existing ones within the target deck, and if a card is too similar to an existing one, the agent will be instructed to write a new card.
